...
// (implementations) after main. This is required so that the C compiler
// knows the type signature of any function used.

// This is the definition (implementation) of add2(). Since it is
// defined before main(), we don't need to provide a prototype for
// add2(). This is why some C programmers put main at the end of the
...
int main(void) {
    ...
    // Note later, when we learn assembly langauge, we will see that
    // in some cases the stack is not necessary for local variables, 
    // but it is useful to think about local variables being allocated
    // on the stack.
    //
    // The nice thing about stack allocated variables (local variables)
    // is that the space for local variables is reclaimed (deallocate)
    // when a fuction returns. So, you don't need to worry about local
